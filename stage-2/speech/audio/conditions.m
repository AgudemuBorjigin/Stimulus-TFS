            ...
            for k = 1:ns(j)
                ...
                switch configuration
                    case {'echo-pitch', 'echo-sum', 'pitch', 'sum'}
                        load(strcat(root_audios, '/target_masker/opposite_gender/trial', int2str(num_trial + (v-1)*numPerVisit), '.mat'));
                        stim = stim_opposite; b = b_opposite;
                    otherwise
                        load(strcat(root_audios, '/target_masker/same_gender/trial', int2str(num_trial + (v-1)*numPerVisit), '.mat'));
